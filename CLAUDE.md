...
Jarvis is offline-first by principle. Every capability works with no network: the local path is both the default and the fallback, and it is never removed to make room for a hosted one. Do not add integrations that depend on a specific proprietary cloud vendor for a capability that already has a working local implementation (e.g. swapping local TTS for ElevenLabs). Generic, self-hostable protocols where the user points at their own endpoint (Ollama, OpenAI-compatible servers, LM Studio) are fine precisely because they can be local; a vendor-locked cloud service used to replace a local one is not. When a feature request asks for such a service, decline it on principle and redirect to the offline equivalent (e.g. better local TTS voices rather than cloud TTS).

One capability has no local equivalent, and is allowed as a bounded exception: realtime speech-to-speech conversation. Sub-second turn-taking and semantic endpointing cannot be reproduced by the local STT/LLM/TTS pipeline at any model size, because the local pipeline must wait out a fixed silence timer before it can start. The exception is bounded by `src/jarvis/realtime/realtime.spec.md`: it ships off by default, the user supplies their own credentials, audio leaves the machine only while a session is explicitly active, and the local pipeline remains the default and the fallback. Nothing outside that boundary inherits the exception.
...
| `src/jarvis/tools/external/mcp_registry.spec.md` | Official MCP registry client, local cache, and the connections directory's trust wording | Metadata-only and browsed from cache so offline still works; the registry proves namespace ownership, never safety; no pinned launch means no Add button |
| `src/jarvis/tools/external/mcp_security.spec.md` | MCP security: launch pinning, tool fingerprinting, the confirmation gate, the local audit, and credential storage | Four independent defences at four moments; server annotations are advisory and cannot relax the user's policy; audit checks are structural, never lexical |
...
| `src/jarvis/realtime/realtime.spec.md` | Realtime speech-to-speech voice: session lifecycle, tool handoff, barge-in, fallback to the local pipeline | Model owns conversation, Jarvis owns capability; the security gate still applies; local pipeline is default and fallback |
