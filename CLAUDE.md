...
| `src/jarvis/audio/audio.spec.md` | Acoustic echo cancellation: playback reference ring, canceller interface, delay calibration | Fail-open to `NullEchoCanceller`; the delay is measured, never guessed; a declined frame is announced, not silently absent |
| `src/jarvis/speech/speech.spec.md` | Speech-to-text provider interface, hosted adapter, language normalisation | Local Whisper is the default and the fallback; `None` means fall back, `""` means silence; hosted transcripts face the same segment filters |
| `src/jarvis/output/sentence_stream.spec.md` | Sentence splitting and `SpeechChunk` bookkeeping for sentence-at-a-time TTS | Callbacks belong to the chunk, not the engine; `total_duration` is the whole reply; splitting is script-aware, never language-listed |
...
| `src/jarvis/tools/selection.spec.md` | Pre-loop tool selection: the four strategies, two-stage embedding retrieval feeding the LLM router, tool-vector cache | Retrieval narrows, the router decides; fails open to the full catalogue in every direction; `stop` is always included |
