...
- **Inputs**:
  ...
  - **Cold-start guidance** (`COLD_START_GUIDANCE` in [src/jarvis/system_prompt.py](src/jarvis/system_prompt.py)), appended only when none of the four memory blocks reached this reply: `not (warm_profile_block or conversation_context or graph_context or memory_digest_text)`. No LLM call, pure branch on the memory locals after #4 settles them. On an empty install the `[Context: ...]` line is the only concrete data in the prompt and the persona's open-ended rule licenses inventing an observation from it, so every greeting becomes a time report; the block rules the clock out as subject matter (scoped to when the user did not ask about it) and supplies two concrete alternatives instead. Dropped as soon as any memory exists, so it never competes with the "lead with a stored fact" rule. See [reply.spec.md](../src/jarvis/reply/reply.spec.md) "Cold Start"
