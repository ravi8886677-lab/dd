...
- **Inputs**:
  ...
  - Unified system prompt from [src/jarvis/system_prompt.py](src/jarvis/system_prompt.py) + ASR note + tool-protocol guidance. Carries the completion-claim rule: a tool result that says it could not confirm the change, or that begins `NOT DONE`, may not be reported as done (see [src/jarvis/audit/audit.spec.md](src/jarvis/audit/audit.spec.md); evals/test_completion_claims.py is the live case)
  ...
  - **Cold-start guidance** (`COLD_START_GUIDANCE` in [src/jarvis/system_prompt.py](src/jarvis/system_prompt.py)), appended only when none of the four memory blocks reached this reply: `not (warm_profile_block or conversation_context or graph_context or memory_digest_text)`. No LLM call, pure branch on the memory locals after #4 settles them. On an empty install the `[Context: ...]` line is the only concrete data in the prompt and the persona's open-ended rule licenses inventing an observation from it, so every greeting becomes a time report; the block rules the clock out as subject matter (scoped to when the user did not ask about it), forbids the model making its own memory the topic (no "nothing is stored yet", no asking the user for things to remember), and supplies two concrete assistant-shaped alternatives instead. Dropped as soon as any memory exists, so it never competes with the "lead with a stored fact" rule. See [reply.spec.md](../src/jarvis/reply/reply.spec.md) "Cold Start"
