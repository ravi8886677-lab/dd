...
- **Replacement material**: banning the clock alone would leave only the bare greeting the persona prompt already forbids, so the guidance names two concrete alternatives (ask what the user is working on or needs, or name one concrete thing the assistant can help with) and requires a different choice each time.
- **It is an assistant, not a diary**: the guidance forbids mentioning memory, the database, what is or is not stored, whether the user is recognised, or asking the user for things to remember. The empty diary is the *reason* this branch exists, never its subject. Soliciting facts to store turns the first thing a new user reads into an interview, and makes a memory system the topic of a greeting; remembering is background to helping, not the service on offer.
